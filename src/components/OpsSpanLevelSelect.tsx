...
interface OpsSpanLevelSelectProps {
  history: HistoryEntry[]
  onSelect: (level: Level) => void
  onBack: () => void
}

export function OpsSpanLevelSelect({
  history,
  onSelect,
  onBack,
}: OpsSpanLevelSelectProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col gap-6 px-4 py-8 sm:px-6 sm:py-12">
      ...
      <LevelPicker
        labelFor={(level) => t.opsSpan.levelLabel(level)}
        colorFor={(level) => LEVEL_STYLES[level]}
        statsFor={(level) => getLevelStats(history, level, 'ops-span')}
        onSelect={onSelect}
      />
    </div>
  )
