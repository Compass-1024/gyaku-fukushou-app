...
**おぼえトレ**（npmパッケージ名は`gyaku-fukushou-app`のまま。旧名称「逆復唱トレーニング」からのリブランディングだが、localStorageキー接頭辞・パッケージ名は既存ユーザーのデータ互換性維持のため変更していない）は、ワーキングメモリ（作業記憶）を鍛えるトレーニングアプリ。「ことば」「すうじ（逆から入力）」「すうじ（合計を入力）」「Nバック」「デュアルNバック」「空間」「変化検出」「音・色」「処理記憶」「ランダム」の10モードを提供する（すうじモードはホーム画面上で2つの独立したモードカードとして提供される）。UIは日本語/英語の2言語に対応（設定画面でいつでも切り替え可能、詳細は「多言語化（i18n）」セクションを参照）。トレーニング機能自体はバックエンドを持たないSPAで、全データはブラウザの`localStorage`に保存する。**例外として、オプトインの「リマインド通知」機能のみ、Vercel Serverless Functions + Redisストレージを使う最小限のバックエンドを持つ**（詳細は「プッシュ通知リマインダー」セクションを参照）。全モード共通で経験値・プレイヤーレベル・今日のミッションといったゲーミフィケーション要素を持つ（詳細は「経験値・プレイヤーレベルシステム」「今日のミッション」セクションを参照）。PWA対応でホーム画面に追加してネイティブアプリ風に利用可能。デプロイ先: https://oboetore.vercel.app/
...
flowchart TD
    ...
    modeSelect["mode-select（個別選択モード画面、10モードカードグリッドからランダムを除く9モード）"]
    ...
    opsSpanLevel["ops-span-level（レベル選択）"]
    opsSpanGame["ops-span-game（ゲーム画面）"]
    opsSpanResult["結果表示"]

    modeSelect -->|処理記憶モード| opsSpanLevel --> opsSpanGame --> opsSpanResult
    opsSpanResult -->|同レベルで再挑戦| opsSpanGame
    opsSpanResult -->|戻る| opsSpanLevel
    opsSpanLevel -->|← モード選択| modeSelect
...
### 処理記憶モード（`src/lib/opsSpan.ts`, `src/hooks/useOpsSpanTrialFlow.ts`）

簡単な暗算の正誤判定（処理課題）と1桁の数字の記憶（記憶課題）を交互に繰り返し、最後に記憶した数字を提示順のまま入力する二重課題ワーキングメモリ課題（Operation Span Taskを参考）。他モードが「記憶するだけ」なのに対し、処理と記憶を同時にこなす負荷の高さが特徴。

- **出題方式**: レベルごとの試行数ぶん、1試行＝「1〜9の一桁同士の暗算(a+b)とその判定用に提示する合計値(shownSum、約50%の確率でわざと±1〜3ずらす)」＋「記憶する0〜9の数字1つ」を生成。3問1セットで出題。固定候補プールを持たないモードのため出題重み付け（`questionWeighting.ts`）は適用しない（ことば以外で重み付けが無いNバック・デュアルNバックと同様）。
- **レベル定義**:
  | レベル | 試行数 |
  |---|---|
  | 1 | 3 |
  | 2 | 4 |
  | 3 | 5 |
- **1問の流れ**（`ready → showing → answering → result`。`showing`の中身は`useOpsSpanTrialFlow`フックが試行ごとに`judging → revealing → gap`の3ステップで進行する）:
  1. `ready`: 1000ms待機
  2. 各試行の`judging`: 暗算の式（例:「3 + 8 = 13」）を表示し、⭕/❌ボタンで正誤判定。制限時間3000ms（`JUDGING_TIMEOUT_MS`）、未回答はタイムアウトで不正解扱いとして次へ進む
  3. 各試行の`revealing`→`gap`: 記憶する数字を1つ表示（700ms＋間隔250ms、`digits.ts`の`DIGIT_SHOWN_MS`/`DIGIT_GAP_MS`を共有）
  4. 全試行が終わると`answering`: テンキーUIで記憶した数字を提示順のまま入力（すうじモード「逆から入力」とは異なり反転しない）。タイムアウトは`digits.ts`と同じ`getAnswerTimeoutMs`（ベース2000ms＋試行数×2000ms）を再利用
  5. `result`: 記憶課題の正誤判定に加え、暗算の正誤判定に何問正解できたかも補足情報として表示（`judgedCorrectCount`。この値は正誤判定・スコアには影響しない）
- **正誤判定**: 記憶した数字を提示順のまま連結した文字列と、入力値を`isDigitStringMatch`（すうじモードと共通のロジック、先頭0埋め許容）で比較。
- アダプティブ難易度モード・セッション状態の一時保存（sessionStorage）には非対応（ことば/Nバック系と同じ理由。試行ごとのユーザー応答を挟む多段状態を外部から正しく復元するにはフック自体の設計変更が必要でコストが高いため、現時点では対象外としている）。

### ランダムモード（`src/lib/random.ts`）

すうじ（逆から入力・合計を入力）・空間・変化検出・音/色・処理記憶・ことばの7候補から出題数分を集め、シャッフルした順で出題するミックス練習モード。Nバック系（連続試行方式）のみ、試行位置が内部stateから導出され外部の状態機に統合しづらいため対象外。
...
- **出題するモードの選択**: `RandomLevelSelect.tsx`で7候補（すうじ・逆から/合計/空間/変化検出/音・色/処理記憶/ことば）のうちどれを対象にするかトグルボタンで選べる（`RandomRoundType`、既定は全種類）。ことばモードは英語UIでは候補から除外する（`availableTypes`）。最後の1つは選択解除できない（`buildRoundGenerators`の`enabledTypes`が空配列になると出題不能になるため、UI側で最小1件を強制する二重防御）。選択状態は`View`（`App.tsx`）の`enabledTypes`として保持し、レベル選択・結果画面間の遷移をまたいで維持される。
- **1ラウンドの流れ**: すうじ/空間/変化検出/音・色の4種類は、出題生成・正誤判定ロジックを対応する`lib/*.ts`からそのまま呼び出し、表示・入力UIのみラウンド種別ごとに`RandomGameScreen.tsx`内で切り替える（結果フェーズの詳細表示は`RandomResultDetail.tsx`が単体モード画面と同じ内容を出し分ける。音・色ラウンドのパッド配色は`src/lib/tonePadStyles.ts`を共有）。処理記憶・ことばの2種類は、音声合成/認識・多段の試行ループという入出力の構造が他4種類（`ready → showing → answering → result`の単純な状態機）と大きく異なるため、`RandomOpsSpanRound.tsx`/`RandomWordRound.tsx`という自己完結したコンポーネント（単体モード画面のGameScreen/OpsSpanGameScreenと同じロジックを1問ぶんだけ切り出したもの）として`RandomGameScreen.tsx`から呼び出す。これらのラウンドでは`RandomGameScreen`側の共通state（`phase`/`useStepReveal`/`useCountdown`等）を`isIsolatedRound`フラグで無効化し、二重に採点が走らないようにしている。
- **正誤判定**: すうじ/空間/変化検出/音・色は元モードの判定関数（`isDigitAnswerCorrect`/`isSpatialAnswerCorrect`/`isPatternSelectionCorrect`/`isToneAnswerCorrect`）をそのまま使う。すうじ（合計）ラウンドの最大入力文字数・自動採点判定は単体のDigitGameScreenと同じ`getDigitMaxAnswerLength`相当のロジックを使う（「逆から」のみ桁数到達で自動採点、「合計」は常に決定ボタンでの明示確定）。処理記憶・ことばは、それぞれの自己完結コンポーネントが単体モード画面と同じ判定ロジックをそのまま使う。
...
- Android実装を見据えたセッション状態復元（`src/lib/gameSessionPersistence.ts`）に対応済み。詳細は「Data model」内の「セッション状態の一時保存」を参照（処理記憶・ことばラウンドの試行途中の状態までは復元されず、再開後はラウンド先頭からやり直す。他モードのセッション復元と同じ制約）。
...
- **ワーキングメモリの伸び**（`src/lib/benchmarks.ts`）: ことば／すうじ（逆から入力）／Nバック／デュアルNバック／空間／変化検出／音・色／処理記憶／ランダムの全9モードについて、一般的な心理学的基準ではなく**ユーザー自身の過去の挑戦履歴**と比較する自己ベンチマーク方式。対象モードの挑戦履歴を古い順に前半/後半へ二分し、それぞれの正答率を比較して「向上中／横ばい／低下ぎみ」の3段階で表示する（差が5パーセントポイント未満なら横ばい扱い）。前半・後半それぞれ最低2回以上の挑戦が無いモードは比較不能として表示しない。標準化された心理学課題を持たないモード（デュアルNバック・ランダム・ことば・音・色・処理記憶）も、自己比較方式のため外部の目安レンジ無しに対象にできる。統計画面ではモード別正答率と同様の2列グリッドでコンパクト表示し、詳細な正答率はtitle属性のツールチップに含める。**医学的な診断や公式な認知機能評価ではない旨の免責文言を常時表示する**（過度に精密な数値や統合スコア化は意図的に避けている）
