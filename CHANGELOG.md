...
- 新モード「処理記憶モード」（`ops-span`）を追加。簡単な暗算の正誤判定（処理課題）と1桁の数字の記憶（記憶課題）を交互に繰り返し、最後に記憶した数字を提示順のまま入力する二重課題ワーキングメモリ課題（Operation Span Taskを参考）。個別選択モード・ランダムモードの両方で挑戦できる（`src/lib/opsSpan.ts`、`useOpsSpanTrialFlow.ts`、`OpsSpanLevelSelect.tsx`/`OpsSpanGameScreen.tsx`）
- ランダムモードにことばモード・処理記憶モードのラウンドを追加。音声合成/認識・二重課題の入出力が他ラウンド（ready→showing→answering→result型）と大きく異なるため、`RandomWordRound.tsx`/`RandomOpsSpanRound.tsx`という自己完結したコンポーネントとして切り出し、`RandomGameScreen.tsx`の共通の状態機とは独立に動作させる設計にした
...
- ホーム画面に「今日のミッション」専用画面を新設。過去の正答率から自動選定した弱点モード・レベルを1日3セット達成すると+100XP。達成後はグレーアウト表示になり達成済みであることが分かる（`src/lib/dailyMission.ts`、`DailyMissionScreen.tsx`。既存の`missions.ts`が持つ`missionCompletions`ログを共有して再利用し、専用のXP付与経路を新設せずに済むよう設計）
- 統計画面に「トレーニングスコア」（総合＋数字記憶/空間記憶/注意制御の3カテゴリ）を追加。直近の正答率をそれ以前の正答率と比較し「前回比 +5pt」のような一言で示す、グラフを使わないシンプルな指標（`src/lib/trainingScore.ts`、`StatsScoreSection.tsx`）
- 実績を2件追加（🎖️ スコアマスター: 総合トレーニングスコア80%到達、🧭 バランストレーナー: 数字記憶/空間記憶/注意制御の3カテゴリすべてに挑戦、計24種類に）
- ランダムモードのレベル選択画面で、出題数に加えて出題するモード（すうじ・逆から/合計・空間・変化検出・音/色）も個別に選べるように変更（`src/lib/random.ts`の`enabledTypes`、最低1つは選択必須）

### Changed

- GitHubリポジトリ名を`gyaku-fukushou-app`から`oboetore`にリネーム（Vercelプロジェクト名のリブランディングと統一。`gh repo rename`実行、GitHub側の旧URLからのリダイレクトは自動で維持される）
- 統計画面の学習カレンダーのマスを拡大（12px→30px）し、直近5週間表示に短縮した際に生じていた余白の多さ・見た目の違和感を解消
- 統計画面の「モード別の正答率」を、常時全モード表示のリストからプルダウンで1モードずつ選んで表示する形式に変更し、画面の縦の長さを圧縮した（`StatsAreaAccuracySection.tsx`）
- ホーム画面を9モードグリッド＋ミッション/お題/7日間チャレンジのチップ行から、[ランダムモード]/[個別選択モード]/[今日のミッション]の3ボタンにシンプル化。9モードのカードグリッド（ランダムモードを除く8モード）は新設の「個別選択モード」画面（`ModeSelectScreen.tsx`）に移動した。既存の「今日のミッション」（日替わり固定9種）「本日のお題」「7日間チャレンジ」はホーム画面から撤去したが、コードは残しておりデータ・XP計算は引き続き動作する（将来の復活に備え、ユーザーとの相談の上での判断）
- 各モードのレベル選択画面の「← モード選択」ボタンの遷移先を、ホーム画面から新設の「個別選択モード」画面に変更（ランダムモードのレベル選択画面のみ、ホーム画面に直接専用ボタンがあるため従来どおりホームへ戻る）
- ホーム画面の説明文から「脳の」を削除（「9種類のトレーニングで、脳のワーキングメモリを鍛えましょう」→「9種類のトレーニングで、ワーキングメモリを鍛えましょう」）。スマートフォンでの2行折り返しを解消
- 統計画面の学習カレンダーの表示期間を、直近18週間（約4ヶ月強）から直近5週間（約1ヶ月）に短縮
- 統計画面から「正答率の推移」「モード別の正答率推移」の折れ線・時系列グラフ、「ワーキングメモリの伸び」（9エリアのベンチマークグリッド）を削除し、上記の「トレーニングスコア」に統合した
- 通知センター（実績・達成の一覧）から、解除・達成から3日を超えて経過した項目を除外するようにした（`getCombinedNotificationLog`）
- アプリ名を「逆復唱トレーニング」から「おぼえトレ」にリブランディング。名称が「逆復唱」（ことばモード1つの操作）に由来し、9モードを擁する現在のアプリ全体を表す名前として実態と合わなくなっていたため。PWAマニフェスト名・ページタイトル・通知タイトル・シェア文言・エクスポートファイル名・公開URL（`https://oboetore.vercel.app/`、Vercelプロジェクトを`gyaku-fukushou-app`から`oboetore`へrename）・GitHubリポジトリ名（`gyaku-fukushou-app`→`oboetore`）を変更。localStorageのキー接頭辞・npmパッケージ名は、既存ユーザーのデータ互換性維持のため変更していない
...
- ホーム画面表示時にチップを自動展開する設定（`AppSettings.autoExpandChip`、設定画面の「チップの自動展開」項目）を削除。対象だった旧ミッション/デイリーチャレンジ/プログラムのチップ行（`TopEngagementChips.tsx`）がホーム画面の3ボタン化で既に呼び出されなくなっており、この設定自体が意味を持たなくなっていたため。付随して`DailyChallengeCard.tsx`・`src/lib/program.ts`もどこからも参照されなくなり削除した

### Fixed

- ホーム画面の見出しが「アプリ名を『逆復唱トレーニング』から『おぼえトレ』にリブランディング」後も汎用的な「ワーキングメモリトレーニング」のままで、ブラウザタブ名（`おぼえトレ`）と食い違っていたため「おぼえトレ」に統一（`src/lib/i18n/ja.ts`の`top.heading`）。英語版も既に確立している表記（OGP/リマインド通知等で使用中の`Oboe-Tore`）に合わせて`Working Memory Training`から`Oboe-Tore`に統一。見出し文言を参照するE2Eテスト（`top.spec.ts`/`navigation.spec.ts`/`language.spec.ts`/`pwa-shortcuts.spec.ts`）も追随
- 処理記憶モード（`ops-span`）の挑戦履歴が、画面遷移のたびに`loadHistory()`のバリデーションで静かに除外され消滅していた重大な不具合を修正。`src/lib/history.ts`の検証用定数`VALID_MODES`（`Mode`型拡張時の同期対象）に`ops-span`が抜けていたことが原因。統計・実績・XP・トレーニングスコア・週間振り返り・自己ベンチマークいずれにも処理記憶モードの記録が反映されない状態だった。同様の同期漏れが型エラーとして検知されるよう、配列決め打ちから`Record<Mode, true>`のオブジェクトリテラルへ実装を変更し、`history.test.ts`にMode型の全バリアントを対象にした回帰テストを追加
- 処理記憶モードを一度もプレイしていなくても「コンプリート」「全モードマスター」実績が解除されてしまう不具合を修正。`src/lib/achievements.ts`の`ALL_MODES_FOR_MASTERY`・`all-eight-modes`の判定ロジックに`ops-span`が含まれていなかったことが原因（上記と同じ同期漏れ）。実績の説明文言も「全8モード」→「全9モード」に修正し、`achievements.test.ts`のテストも9モード仕様に更新
- オンボーディング・PWAマニフェスト・OGP/Twitter Card用のメタ説明文が「9種類のゲーム」のまま処理記憶モード追加後も未更新だったため、「10種類」に統一（`src/lib/i18n/ja.ts`, `vite.config.ts`, `index.html`）
...
- 統計画面の実績解除ページで、`bg-emerald-700`のレベル1ボタン配色がTailwindのバージョン更新後にWCAG AA基準（コントラスト比4.5:1）を下回っていた不具合を修正（`emerald-800`に変更）
