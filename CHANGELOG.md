...
- モードを途中でやめて（← レベル選択）再挑戦した際、直前のセットで実際に表示済みだった問題が再び出題されないようにする仕組みを追加。ことば/すうじ（逆から入力・合計を入力）/空間/変化検出/音・色/ランダムの全モードで対応（`src/lib/recentQuestions.ts`、sessionStorageに一度きりの除外候補として保存。`questionWeighting.ts`の`pickWeightedCandidate`にexclude/isEqual引数を追加し、ランダムモードは弱点重視モードでも正しいレベルへ除外が効くよう単体モード画面と同じキー（`<mode>:<level>`）を共有する設計）

### Changed

- 結果画面（SetSummary）の全問内訳リストを、Nバック/デュアルNバック（最大30試行＝最大60行）のように出題数が多いモードでは既定で折りたたみ表示にし、「内訳を表示」ボタンで必要な人だけ展開できるようにした。従来は結果画面を開くたびに大量の行が並び、スコア・実績などの演出バナーが埋もれて「次に何をすべきか」が分かりにくかった。3問1セットの他モード（ことば/すうじ/空間/変化検出/音・色/ランダム）は従来どおり常に展開表示（`SetSummary.tsx`のITEM_LIST_COLLAPSE_THRESHOLD、8問超で折りたたみ）

### Fixed

- ランダムモードのすうじ（逆から入力）・空間・音/色ラウンドで、自動採点（桁数/系列長に達した時点での判定）が直前の1手を反映する前の古い状態を参照しており、正しく回答しても不正解と判定される不具合を修正。特に音/色ラウンドでは常に最後の1タップが欠けた状態で判定されるため、実質的に系列長より1つ少ない回数分しか正しく認識されなかった（`RandomGameScreen.tsx`の`finalizeAnswer`を、`setState`の関数型アップデータ内から呼ぶ際は最新値を明示的に引数で渡す設計に変更。単体のDigitGameScreen/ToneGameScreen等と同じ設計に統一）
- ランダムモードのすうじラウンドで、単体のDigitGameScreenにはある回答フェーズ中の物理キーボードでの数字入力（0-9・Backspace・Enter）が一切効かず、画面タップでしか入力できなかった不具合を修正
- すうじ/空間/変化検出/音・色の4モードで、1セット3問のうち同じ問題（同じ数字列・系列・模様）が重複して出題されうる不具合を修正。ことばモードは元々フレーズプールから重複排除して選んでいたが、固定候補プールを持たない他4モードは1問ずつ独立に乱数生成しており、低確率ながら3問中2問が同一になりうる状態だった。同一セット内で既に選んだ問題も除外対象に含めるよう`pick*QuestionSet`を修正（`digits.ts`/`spatial.ts`/`pattern.ts`/`tone.ts`）

## [0.9.0] - 2026-08-10

### Fixed

- ランダムモードの結果フェーズで、単体モード画面（DigitGameScreen等）と異なり正誤バッジのみが表示され「出題」「正しい答え」「あなたの回答」といった詳細が一切表示されていなかった不具合を修正（`RandomResultDetail.tsx`を新設し、ラウンド種別ごとに単体モード画面と同じ内容を表示）
- ランダムモードのすうじ（合計を入力）ラウンドで、最大入力文字数と自動採点の判定が「逆から入力」用のロジック（出題の桁数に達したら自動採点）のまま使い回されており、単体のすうじモード画面と異なり合計の桁数を考慮していなかった不具合を修正（`getDigitMaxAnswerLength`、合計ラウンドは決定ボタンでの明示確定のみに統一）
- ランダムモードの音・色ラウンドで、4色のパッドがすべて同一色（インジゴ）で表示され、単体の音・色モード画面のような色分け（赤/青/緑/黄）がされていなかった不具合を修正（`src/lib/tonePadStyles.ts`に配色を共通化）
- ランダムモードの空間・変化検出ラウンドのマスに、単体モード画面にはある`aria-label`/`aria-pressed`が付いておらず、スクリーンリーダーでマスの状態が伝わらなかった不具合を修正
- ランダムモードの空間ラウンドのマスサイズが単体モード画面と異なっていた（48px→56pxに統一）
- ランダムモードでレベル提案（「🎉 次のレベルへ挑戦」等）を選んだ際、弱点重視オプション・出題数の設定が引き継がれずリセットされていた不具合を修正

### Added

- ランダムモードの出題数を3/5/7問から選べるようにした（`src/lib/random.ts`のROUND_COUNT_OPTIONS、Nバックの出題数選択と同じ設計。5種類の候補ラウンドのうち3問なら重複無く一部を選び、7問なら超過分だけランダムに重複させる）
- ランダムモードにもバックグラウンド復帰後のセッション状態復元を追加（すうじ/空間/変化検出/音・色の4モードと同じ`gameSessionPersistence.ts`を利用）
...
- ホーム画面のサブタイトルから「楽しく」を削除し、より簡潔な1行の文言に変更
- ホーム画面のモードカード説明文を全体的に短縮し、スマートフォン幅でも2行に折り返さないよう`line-clamp-1`に変更
- ホーム画面のプレイヤーLv・今日の目標を、縦積みの2ブロックから横並び2カラムの1行にまとめてコンパクト化
- ホーム画面の「今日のおすすめ」カードを削除
- ホーム画面の「今日のミッション」「本日のお題」「7日間チャレンジ」を、常時全文表示の3枚のカードから、横並びのコンパクトなチップに変更。タップした項目のみその場で詳細を展開する方式にした
